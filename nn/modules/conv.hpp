...
class Conv2d : public infinidemo::nn::modules::Module<Conv2d> {
public:
    Conv2d(int in_channels, int out_channels, size_t kernel_size,
        ...
        INFINICORE_NN_PARAMETER_INIT(
            weight, ({static_cast<size_t>(out_channels),
                      static_cast<size_t>(in_channels), kernel_size, kernel_size},
                     dtype_, device_));
        if (bias) {
            INFINICORE_NN_PARAMETER_INIT(
                bias, ({static_cast<size_t>(out_channels)}, dtype_, device_));
        } else {
            bias_ = infinicore::nn::Parameter(); // Default constructed empty parameter
        }
    }

    inline Tensor forward(Tensor &input) const {
        std::vector<size_t> pads = {padding_, padding_};
        std::vector<ptrdiff_t> strides = {static_cast<ptrdiff_t>(stride_),
                                          static_cast<ptrdiff_t>(stride_)};
        std::vector<size_t> dilations = {dilation_, dilation_};
        std::vector<size_t> output_shape = computeConv2dOutputShape(
            input->shape(), weight_->shape(), pads, strides, dilations);
...
private:
    void to_device_(const Device &device) {
...
protected:
    ...
    std::vector<size_t> computeConv2dOutputShape(
        ...
        for (size_t i = 0; i < spatial_dims; i++) {
            ...
            // Formula: output_size = (input_size + 2*pad - dilation*(kernel_size-1) -
            // 1) / stride + 1 Equivalent to: (padded_input - effective_kernel) /
            // stride + 1 where padded_input = input_size + 2*pad and effective_kernel
