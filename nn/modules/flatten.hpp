...
public:
  ...
  inline Tensor forward(Tensor &input) const {
    ...
    if (start_dim_ < 0 || start_dim_ >= ndim || actual_end_dim < 0 ||
        actual_end_dim >= ndim || start_dim_ > actual_end_dim) {
    ...
    new_shape.insert(new_shape.end(), shape.begin(),
                     shape.begin() + start_dim_);
    new_shape.push_back(flattened_size);
    new_shape.insert(new_shape.end(), shape.begin() + actual_end_dim + 1,
                     shape.end());
